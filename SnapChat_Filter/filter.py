import sys
import cv2, time, dlib, mathimport 
import numpy as np
...
def calc_incl(p1,p2):
    x1,x2,y1,y2 = p[1], p2[0], p1[1], p2[1]
...
def draw_sprite(frame, sprite, x_off, y_off):
    ...
    (imW,imH) = (frame.shape[0], frame.shape[1])
    ...
    for c in range(3):
        frame[y_off:y_off+h, x_off:x_off+w, c] = sprite[:,:,c] * (sprite[:,:,3]/255.0) + frame
        return frame
    ...
    def get_bbox(points, part):
        if part == 1:
            (x,y,w,h) = calc_boox(points[17:22]) #Left Eyebrow
        elif part == 2:
...
while True:
    ...
    for face in faces:
        ...
        incl = calc_incl(shape[17], shaper[26])
        ...
        apply_sprite(frame, './sprites/noespick.png', w2, x2-15, y2+25, incl, ontop = False)
        if mouth_open:
